...
`BiocSet` also allows for common set operations to be performed on the `BiocSet`
 object. `union()` and `intersection()` are the two se operations available in 
`BiocSet`. We demonstate how a user can find the union (or intersection) between
 two `BiocSet` objects or within a single `BiocSet` object.
