...
`BiocSet` can create a `BiocSet` object using two different input methods. The first is to input named character vectors of element sets. `BiocSet` returns three tibbles, `element` which contains the elements, `set` which contains the sets and `elementset` which contains elements and sets together. 
...
Next, we demonstrate the use of `BiocSet` with an experiment dataset `airway` from the package `airway`. This data is from an RNA-Seq experiment on airway smooth muscle (ASM) cell lines.
...
The function `go_sets()` discovers the keys from the org object and uses `AnnotationDbi::select` to create a mapping to GO ids. We represent these identifieres as a `BiocSet` object. Using the `go_sets` function we are able to map the Ensembl ids and GO ids from the genome wide annotation for Human data in the `org.Hs.eg.db` package. The Ensembl ids are treated as elements while the GO ids are treated as sets.
...
To simplify mapping we created a function called `es_map`. This takes four arguements, a `BiocSet` object, an `AnnotationDbi` object, the id to map from, and the id to map to. The function utilizes `mapIds` from `AnnotationDbi` and returns a `BiocSet` object. In the example below we show how to use this function to map the Ensembl ids from `go` to gene symbols.
...
A final feature to `BiocSet` is the ability to look up information about the different elements/sets. A user could utilize the function `url_ref()` to get more information about a specific id. Below is an example of looking up information for one of the Ensembl ids (which is an element) and one of the GO ids (which is a set) in the `go` data set we worked with above.
