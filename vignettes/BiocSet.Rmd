...
`BiocSet` also provides functionality for mapping different ID types and being 
directed to a reference webpage for specific elements/sets.
...
three tibbles, `element` which contains the elements, `set` which contains the 
sets and `elementset` which contains elements and sets together. 
...
The second method of creating a `BiocSet` oject would be to read in a `.gmt` 
...
`BiocSet` includes a `source` column within the `elementset` tibble for 
...
`BiocSet` adopts the use of many `dplyr` functions such as `filter()`, 
`select()`, `mutate()`, `summarise()`, `arrange()`, and `tbl_vars()`.
...
Another feature available to `BiocSet` is the ability to activate different 
tibbles. When a `BiocSet` is created, the tibble `elementset` is automatically 
activated and all functions will be performed on this tibble. With each of the 
functions the user is able to pick a different tibble to activate and work on 
by using 'verb_tibble'. After the function is executed than the 'active' tibble
 is returned back to the tibble that was active before the function call. Some 
examples are shown below of how these functions work.
...
Next, we demonstrate the use of `BiocSet` with an experiment dataset `airway` 
from the package `airway`. This data is from an RNA-Seq experiment on airway 
...
`AnnotationDbi::select` to create a mapping to GO ids. We represent these 
identifieres as a `BiocSet` object. Using the `go_sets` function we are able to
 map the Ensembl ids and GO ids from the genome wide annotation for Human data 
in the `org.Hs.eg.db` package. The Ensembl ids are treated as elements while 
the GO ids are treated as sets.
...
filtering out the non-descriptive elements.
...
library(GO.db)
map <- map_add(go, GO.db, "GOID", "DEFINITION")
...
function that utilizes `KEGGREST` to develop a BiocSet that contains the 
elements for every pathway map in KEGG.
...
kegg <- kegg %>% mutate_element(
            ensembl = map_add(kegg, org.Hs.eg.db, "ENTREZID", "ENSEMBL")
            )
```
...
A final feature to `BiocSet` is the ability to look up information about the 
different elements/sets. A user could utilize the function `url_ref()` to get 
more information about a specific id. Below is an example of looking up 
information for one of the Ensembl ids (which is an element) and one of the GO 
ids (which is a set) in the `go` data set we worked with above.

```{r url, eval = FALSE}
go
url_ref("ENSG00000151729")
url_ref("GO:0000003")
```
