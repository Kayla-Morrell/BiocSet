...
package: ElementSet
vignette: >
    %\VignetteIndexEntry{ElementSet: Representing Element Sets in the Tidyverse}
...
# Introduction
`BiocSet` is a package that represents element sets in a tibble format with the `ElementSet` class. Element sets are read in and converted into a tibble format. From here, typical `dplyr` operations can be performed on the tibble element set.
...
`BiocSet` can create an `ElementSet` using two different input methods. The first is to input named character vectors of element sets. `ElementSet` returns three tibbles, `element` which contains the elements, `set` which contains the sets and `elementset` which contains elements and sets. 
...
The second method of creating an `ElementSet` would be to read in `.gmt` files. Using `import()`, a path to a downloaded `.gmt` file is read in and an `ElementSet` is returned. The example below uses a hallmark element set downloaded from [GSEA][], which is also included with this package. This `ElementSet` includes a `source` column within the `elementset` tibble for reference as to where the element set came from.
...
`export()` allows for an  `ElementSet` to be exported into a temporary file with the extention `.gmt`.
...
`ElementSet` adopts the use of many `dplyr` functions such as `filter()`, `select()`, `mutate()`, `summarise()`, `arrange()`, and `tbl_vars()`.
...
Another feature available to `ElementSet` is the ability to activate different tibbles. When an `ElementSet` is created, the tibble `elementset` is automatically activated and all functions will be performed on this tibble. With `es_activate()` the user is able to pick a different tibble to activate and work on.
...
Next, we demonstrate the use of `ElementSet` with an experiement dataset `airway` from the package `airway`. This data is from an RNA-Seq experiment on airway smooth muscle (ASM) cell lines.
...
This function `go_sets()` discovers the keys from the org object and uses `AnnotationDbi::select` to create a mapping of Ensembl ids and GO ids. We represent these identifieres as a ElementSet.
    ...
    function(org = org.Hs.eg.db)
{
    ...
    do.call(ElementSet, split(map$ENSEMBL, map$GO))
}
```
...
When creating the KEGG pathway ElementSet we removed pathways that did not contain any elements. Since we want to add in the pathway names to our ElementSet we must first filter out the pathways that were removed. Then we can left join the pathways with the set tibble. We then mutate the element tibble to contain both Ensembl and Entrez ids. 
...
tbl <- es_activate(tbl, element) %>% 
    mutate(
        ensembl = mapIds(
            org.Hs.eg.db,
            keys = es_element(tbl)$element,
            column = "ENSEMBL",
            keytype = "ENTREZID",
            multivals = "first"
        )
...
```{r subset, eval = FALSE}
asthma <- tbl %>% es_activate(set) %>% filter(value == "Asthma")
...
multipaths <- tbl %>% es_activate(set) %>% filter(set %in% pathways)
